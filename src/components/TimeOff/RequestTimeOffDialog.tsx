...
const RequestTimeOffDialog = (props: RequestTimeOffProps) => {

    const [dialogVisible, setIsVisible] = useState(false);
    const [selection, setSelection] = useState("Time off");
  ...
  return (
    <div>
    <button onClick = {() => setIsVisible((prevState) => {return !prevState})}className="main-button big-button" style={{display: 'flex', flexDirection: 'row', gap: '0.5rem', alignItems: 'center', cursor: 'pointer'}}>
    ...
    </button>  
    {
        ...
        <div style={{position: 'absolute', top: 0, left: 0, display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: '100%', zIndex: '20', height: '100%'}}>
            ...
            <div style={{maxWidth: '250px', minWidth: '250px', display:'flex', flexDirection: 'column', gap: '0.5rem', position:'absolute', top: 40, backgroundColor: '#FFFFFF', padding: '2rem 1rem', borderRadius: '1rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)'}}>
                <button style={{backgroundColor: '#FFFFFF', color: '#AEAEB8' ,border: 'none', position: 'absolute', top: '1rem', right: '0.5rem', cursor: 'pointer'}} onClick={() => {setIsVisible(false)}}>
                    <XIcon size={18}/>
                </button>
                ...
                <select className='dropdown-btn' autoFocus onChange={handleSelectionChange}>
                    <option className = 'dropdown-option' value="Time off">Time off - Discounts time and needs approval</option>
                    <option value="Overtime compensation">Overtime compensation - Dost not discount time and needs approval</option>
                    <option value="Medical absence	">Medical absence - Dost not discount time and needs approval</option>
                    <option value="Maternity leave">Paid leave: Maternity leave - Does not discount time and needs approval</option>
                </select>
                ...
                <div style={{zIndex: '50'}}>
                    <SelectDateRange offDays={props.offDays} daysLeft={0} type={selection}/>
                </div>
            </div>
        </div>
    :
